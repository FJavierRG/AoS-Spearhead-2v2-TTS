...
Yoffset=0.1
D3_DICE_IMAGE="https://steamusercontent-a.akamaihd.net/ugc/9940196010924420731/288481E10FCEC69089410E5A71100373E6925EE6/"
...
quick1d6Btn={
    label="Run", click_function="quickRoll1", function_owner=self,
    position={-1.5, Yoffset ,-1.17}, rotation={0,0,0}, height=50, width=250,
    font_size=60, color="Purple", font_color="White"
}
quickD3Btn={
    label="1D3", click_function="quickRollD3", function_owner=self,
    position={-1, Yoffset ,-1.17}, rotation={0,0,0}, height=50, width=250,
    font_size=60, color="Yellow", font_color="Black"
}
add2d6Btn={
...
quick2d6Btn={
    label="Charge", click_function="quickRoll2", function_owner=self,
    position={-0.5, Yoffset ,-1.17}, rotation={0,0,0}, height=50, width=250,
    font_size=60, color="Purple", font_color="White"
}
add3d6Btn={
...
addNd6Btn={
    ...
    position={0.08, Yoffset ,-1.17}, rotation={0,0,0}, height=50, width=340,
    font_size=60, color={1,1,1}, font_color={0,0,0}
}
addNd6Input = {
    label = "How many?", input_function = "setCustomSpawnCount", function_owner = self,
    position = {0.85, Yoffset, -1.17}, rotation = {0, 0, 0},
...
add4d6Btn={
    ...
    position={0, Yoffset ,-0.87}, rotation={0,0,0}, height=50, width=250,
    font_size=60, color={1,1,1}, font_color={0,0,0}
}
add5d6Btn={
...
lowerUpperBtn={
    ...
    position={-5.625, Yoffset ,1.1}, rotation={0,0,0}, height=1000, width=1500, scale={0.1,0.1,0.1},
...
autoWoundsBtn = {
    label="Auto-wounds", tooltip="Move 6s off the mat; right-click to move 5s too", click_function="autoWoundsAction", function_owner=self,
    position={-4.865, Yoffset, 1.1}, rotation={0,0,0}, height=1000, width=3200, scale={0.1,0.1,0.1},
    font_size=500, color="White", font_color={0,0,0}
}
extraHitsBtn = {
    label="Extra Hits", tooltip="Duplicate 6s; right-click to duplicate 5s too", click_function="extraHitsAction", function_owner=self,
    position={-4.225, Yoffset, 1.1}, rotation={0,0,0}, height=1000, width=2500, scale={0.1,0.1,0.1},
    font_size=500, color="White", font_color={0,0,0}
}
autoWoundsAndExtraHitsBtn = {
    label="Auto-wounds & Extra Hits", tooltip="Move 6s off the mat and duplicate them; right-click to move 5s too",
    click_function="autoWoundsAndExtraHitsAction", function_owner=self,
    position={-3.315, Yoffset, 1.1}, rotation={0,0,0}, height=1000, width=5810, scale={0.1,0.1,0.1},
    font_size=500, color="White", font_color={0,0,0}
}
...
function autoWoundsAction(obj, player, alt)
    autoWoundsAndOrExtraHitsAction(player, true, false, alt)
end

function extraHitsAction(obj, player, alt)
    autoWoundsAndOrExtraHitsAction(player, false, true, alt)
end

function autoWoundsAndExtraHitsAction(obj, player, alt)
    autoWoundsAndOrExtraHitsAction(player, true, true, alt)
end

function autoWoundsAndOrExtraHitsAction(player, autoWound, extraHit, alt)
    local crits = getCriticalHits(alt)
    local z_delta = (whichDiceMat == "Red") and -3.1 or 3.1
    ...
    if #crits > 0 then
        if autoWound then
            ...
            printToColor("Moved " .. #crits .. " auto wounds off the mat.", player, "Yellow")
        end
        if extraHit then
            spawnDice(#crits, player)
            printToColor("Generated " .. #crits .. " extra hits.", player, "Yellow")
        end
    else
...
function copyOpponentsDice(obj, player, alt)
    ...
    local position = otherMat.getPosition()
    ...
    for _, die in ipairs(otherDice) do
        ...
        local relPos = {
            x = -(diePos.x - position.x),
            y = diePos.y - position.y,
            z = -(diePos.z - position.z)
        }
...
function quickRollD3(obj, player, alt)
    quickRollD3Physical(player)
end

function quickRollD3Physical(player)
    local directionMultiplier = (whichDiceMat == "Blue") and -1 or 1
    local spawnRotationY = (whichDiceMat == "Red") and 0 or 180
    local total = math.random(3)
    ...
    local die = spawnObjectData({
        data = {
            Name = "Custom_Dice",
            Transform = {
                posX = nextSpawnPosition.x,
                posY = nextSpawnPosition.y,
                posZ = nextSpawnPosition.z,
                rotX = 0,
                rotY = spawnRotationY,
                rotZ = 0,
                scaleX = 1,
        ...
        callback_function = function(spawnedDie)
            Wait.frames(function()
                if spawnedDie ~= nil then
                    -- The saved D3 object's RotationValues label 2/3 opposite to this texture.
                    -- Use explicit rotations so the chat result and visible face stay aligned.
                    local faceRotations = {
                        [1] = Vector(-90, 180, 0),
                        [2] = Vector(0, 0, 0),
                        [3] = Vector(0, 0, -90)
                    }
                    local finalRotation = faceRotations[total] + Vector(0, spawnRotationY, 0)
                    if total == 2 then
                        spawnedDie.setRotationSmooth(Vector(0, spawnRotationY + 120, 0), false, false)
                        Wait.frames(function()
                            if spawnedDie ~= nil then
                                spawnedDie.setRotationSmooth(Vector(0, spawnRotationY + 240, 0), false, false)
                            end
                        end, 20)
                        Wait.frames(function()
                            if spawnedDie ~= nil then
                                spawnedDie.setRotationSmooth(finalRotation, false, false)
                            end
                        end, 40)
                    else
                        local intermediateRotation = Vector(90, spawnRotationY + 180, 90)
                        spawnedDie.setRotationSmooth(intermediateRotation, false, true)
                        Wait.frames(function()
                            if spawnedDie ~= nil then
                                spawnedDie.setRotationSmooth(finalRotation, false, true)
                            end
                        end, 12)
                    end
                end
    ...
    if die then
        ...
        if diceCount >= 5 then
            ...
            nextSpawnPosition.x = nextSpawnPosition.x + directionMultiplier * 1.0
        end
    else
        printToColor("Failed to spawn a D3", player, "Red")
        return
    end

    printToAll(player .. " quick-rolled " .. total .. " on 1D3", "White")
end
...
function onload()
    ...
    self.createButton(quickD3Btn)
    self.createButton(quick2d6Btn)
    self.createButton(copyOpponentsDiceBtn)
    self.createButton(autoWoundsBtn)
    self.createButton(extraHitsBtn)
    self.createButton(autoWoundsAndExtraHitsBtn)
